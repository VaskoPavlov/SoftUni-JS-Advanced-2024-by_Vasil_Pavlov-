/**
 * Task: Create a class Stringer, which holds the single string and a length property. 
...
Implement function increase(length) and decrease(length), which manipulate the length property 
with the given value.
The length property is a numeric value and should not fall below 0. It should not throw any errors, 
but if an attempt to decrease it below 0 is done, it should be automatically set to 0.
...
the string should be cut from right to left, so that it has the same length as the length property, 
